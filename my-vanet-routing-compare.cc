 ...
 *   ./waf --run "vanet-routing-compare --mobility=1"
...
WifiApp::Simulate (int argc, char **argv)
{
  ...
  ConfigureNodes ();
  ConfigureChannels (); //call SetupAdhocDevices()
  ConfigureDevices ();
  ConfigureMobility ();//Call SetupAdhocMobilityNodes
  ConfigureApplications ();// Call SetupRoutingMessages () and SetupWaveMessages ()
  ConfigureTracing ();// Call WriteCsvHeader (), SetupLogFile (), and SetupLogging ()
  RunSimulation (); // Call run()
  ProcessOutputs (); // Calculate and Shows the console output PDR, Throughput etc.
}
 ...
 */
//class VanetRoutingExperiment inherits class WifiApp in public
...
VanetRoutingExperiment::VanetRoutingExperiment ()
  : m_port (9),
    ...
    m_waveInterval (0.1),
	//m_waveInterval (0.0000001),
    m_verbose (0),
    m_scenario (2), //m_scenario=1 for synthetic; m_scenario=2 for trace file
    m_gpsAccuracyNs (40),
    m_txMaxDelayMs (10),
    m_routingTables (0),
    m_asciiTrace (0),
...
VanetRoutingExperiment::ConfigureApplications ()
{
  ...
  SetupRoutingMessages (); //For transmitting IP packet
  SetupWaveMessages ();
...
{
  //WriteCsvHeader (); //write header in the file
  SetupLogFile ();
  SetupLogging ();
...
VanetRoutingExperiment::ProcessOutputs ()
{
  // calculate and output final results
	...
	/**Nawaz**/
	  //int bsm_pdr1_sent =m_waveBsmHelper.GetWaveBsmStats()->GetExpectedRxPktCount(1);
	  //int bsm_pdr1_rcv =m_waveBsmHelper.GetWaveBsmStats()->GetRxPktInRangeCount(1);
	  int bsm_pdr1_sent = m_waveBsmHelper.GetWaveBsmStats ()->GetExpectedRxPktCount (1);
	   int bsm_pdr1_rcv = m_waveBsmHelper.GetWaveBsmStats ()->GetRxPktInRangeCount (1);
	  /**End Nawaz**/
  ...
  if (m_log != 0)
    {
      NS_LOG_UNCOND ("BSM_PDR1=" << bsm_pdr1<< " BSM_sent1=" << bsm_pdr1_sent<< " BSM_rcv1=" << bsm_pdr1_rcv<< " BSM_PDR2=" << bsm_pdr2 << " BSM_PDR3=" << bsm_pdr3 << " BSM_PDR4=" << bsm_pdr4 << " BSM_PDR5=" << bsm_pdr5 << " BSM_PDR6=" << bsm_pdr6 << " BSM_PDR7=" << bsm_pdr7 << " BSM_PDR8=" << bsm_pdr8 << " BSM_PDR9=" << bsm_pdr9 << " BSM_PDR10=" << bsm_pdr10 << " Goodput=" << averageRoutingGoodputKbps << "Kbps MAC/PHY-oh=" << mac_phy_oh);

    }

  std::ofstream out (m_CSVfileName2.c_str (), std::ios::app); //app //ate
...
VanetRoutingExperiment::Run ()
{
  ...
  //AnimationInterface
  //AnimationInterface::SetBoundary(-150, 0, 150, 18);
  //AnimationInterface::SetBoundary (0, 0, 35, 35); // Optional
  AnimationInterface anim ("scratch/300X18animation.xml");
...
VanetRoutingExperiment::CheckThroughput ()
{
  ...
  /*Nawaz*/
  int sent1 = m_waveBsmHelper.GetWaveBsmStats ()->GetExpectedRxPktCount (1);
  int rcv1 = m_waveBsmHelper.GetWaveBsmStats ()->GetRxPktInRangeCount (1);
  int sent2 = m_waveBsmHelper.GetWaveBsmStats ()->GetExpectedRxPktCount (2);
  int rcv2 = m_waveBsmHelper.GetWaveBsmStats ()->GetRxPktInRangeCount (2);
  int sent3 = m_waveBsmHelper.GetWaveBsmStats ()->GetExpectedRxPktCount (3);
  int rcv3 = m_waveBsmHelper.GetWaveBsmStats ()->GetRxPktInRangeCount (3);
  int sent4 = m_waveBsmHelper.GetWaveBsmStats ()->GetExpectedRxPktCount (4);
  int rcv4 = m_waveBsmHelper.GetWaveBsmStats ()->GetRxPktInRangeCount (4);
  int sent5 = m_waveBsmHelper.GetWaveBsmStats ()->GetExpectedRxPktCount (5);
  int rcv5 = m_waveBsmHelper.GetWaveBsmStats ()->GetRxPktInRangeCount (5);
  int sent6 = m_waveBsmHelper.GetWaveBsmStats ()->GetExpectedRxPktCount (6);
  int rcv6 = m_waveBsmHelper.GetWaveBsmStats ()->GetRxPktInRangeCount (6);
  int sent7 = m_waveBsmHelper.GetWaveBsmStats ()->GetExpectedRxPktCount (7);
  int rcv7 = m_waveBsmHelper.GetWaveBsmStats ()->GetRxPktInRangeCount (7);
  int sent8 = m_waveBsmHelper.GetWaveBsmStats ()->GetExpectedRxPktCount (8);
  int rcv8 = m_waveBsmHelper.GetWaveBsmStats ()->GetRxPktInRangeCount (8);
  int sent9 = m_waveBsmHelper.GetWaveBsmStats ()->GetExpectedRxPktCount (9);
  int rcv9 = m_waveBsmHelper.GetWaveBsmStats ()->GetRxPktInRangeCount (9);
  int sent10 = m_waveBsmHelper.GetWaveBsmStats ()->GetExpectedRxPktCount (10);
  int rcv10 = m_waveBsmHelper.GetWaveBsmStats ()->GetRxPktInRangeCount (10);
  ...
  if (m_log != 0 )
    {
      NS_LOG_UNCOND ("At t=" << (Simulator::Now ()).GetSeconds () << " wavePktsSent= "<<wavePktsSent <<" wavePktsReceived= "<<wavePktsReceived<<" PDR="<< wavePDR << " BSM_PDR1=" << wavePDR1_2 << " BSM_PDR1=" << wavePDR2_2 << " BSM_PDR3=" << wavePDR3_2 << " BSM_PDR4=" << wavePDR4_2 << " BSM_PDR5=" << wavePDR5_2 << " BSM_PDR6=" << wavePDR6_2 << " BSM_PDR7=" << wavePDR7_2 << " BSM_PDR8=" << wavePDR8_2 << " BSM_PDR9=" << wavePDR9_2 << " BSM_PDR10=" << wavePDR10_2 << " Goodput=" << kbps << "Kbps" /*<< " MAC/PHY-OH=" << mac_phy_oh*/);
    }
...
VanetRoutingExperiment::SetConfigFromGlobals ()
{
  ...
  GlobalValue::GetValueByName ("VRCnodeSpeed", doubleValue);
  m_nodeSpeed = doubleValue.Get();
...
VanetRoutingExperiment::SetGlobalsFromConfig ()
{
  ...
  g_nodeSpeed.SetValue (DoubleValue (m_nodeSpeed));
...
VanetRoutingExperiment::SetupAdhocMobilityNodes ()
{
  if (m_mobility == 1)
  ...
  else if (m_mobility == 2)
    {
      ...
      //For Rahim settings 100*100 network
      pos.Set ("X", StringValue ("ns3::UniformRandomVariable[Min=-150.0|Max=150.0]"));
      pos.Set ("Y", StringValue ("ns3::UniformRandomVariable[Min=0.0|Max=18.0]"));
     ...
     /* mobilityAdhoc.SetMobilityModel ("ns3::RandomWaypointMobilityModel",
      ...
      mobilityAdhoc.SetMobilityModel("ns3::ConstantPositionMobilityModel"); //constant position mobility model
     // mobilityAdhoc.SetMobilityModel("ns3::RandomDirection2dMobilityModel","Speed", StringValue (ssSpeed.str ()), "Bounds", RectangleValue (Rectangle (-160, 160, 0, 20))); //constant position mobility model
      ...
      WaveBsmHelper::GetNodesMoving ().resize (m_nNodes, 1); //Set Moving or not moving
    }
...
VanetRoutingExperiment::SetupAdhocDevices ()
{
 ...
 /*Default Delay model: ns3::ConstantSpeedPropagationDelayModel*/
  /*Default Propagation loss model: ns3::LogDistancePropagationLossModel*/
  ...
  if (m_lossModel == 3)
    {
	  ...
	  //m_lossModelName = 3 = "ns3::TwoRayGroundPropagationLossModel";
	  //wifiChannel.AddPropagationLoss (m_lossModelName, "Frequency", DoubleValue (freq), "HeightAboveZ", DoubleValue (1.5));
	  wifiChannel.AddPropagationLoss("ns3::LOSnNLOSPropagationLossModel");
    }
  else
    {
  ...
  //Config::Set ("/NodeList/*/DeviceList/*/$ns3::WifiNetDevice/Mac/Slot", TimeValue (MicroSeconds (13)));
  //Config::Set ("/NodeList/*/DeviceList/*/$ns3::WifiNetDevice/Mac/Sifs", TimeValue (MicroSeconds (32)));
}
  ...
  if (m_scenario == 1)
    {
      ...
      m_traceFile = ""; //use mobility.tcl here
      m_logFile = "";
      m_mobility = 2;
      //if (m_nNodes == 156)
       // {
         // m_nNodes = 40;
    	  //m_nNodes = 210;
    	  m_CSVfileName = "scratch/PRP_Tx_A3_Rx_G.csv";
    	  m_CSVfileName2 = "scratch/PRP_Tx_A3_Rx_G.csv";
       // }
      if (m_TotalSimTime == 300.01)
        {
          //m_TotalSimTime = 10.0;
    	  m_TotalSimTime = 30.01;
        }
    }
  else if (m_scenario == 2)
    {
      ...
      m_logFile = "low99-ct-unterstrass-1day.filt.7.adj.log";
	  //m_traceFile ="scratch/cross1lmobility1.tcl";
	  //m_traceFile ="scratch/ntumobility.tcl";
	  //m_traceFile ="scratch/mobility.tcl";
	  //m_traceFile ="src/wave/examples/mobility.tcl";
	  m_logFile="scratch/main-ns2-mob.log";
	  /*Nawaz*/
      ...
      m_CSVfileName = "withoutRelay.csv";
      m_CSVfileName2 = "withoutRelay2.csv";
    }
}
...
VanetRoutingExperiment::WriteCsvHeader ()
{
  ...
  std::ofstream out2 (m_CSVfileName2.c_str (), std::ios::app); //ate, app
  out2 << "BSM_PDR1,"
       << "BSM_PDR2,"
       << "BSM_PDR3,"
       << "BSM_PDR4,"
...
{
//LogComponentEnable ("FunctionCall", LOG_LEVEL_FUNCTION); //LOG_LEVEL_FUNCTION to see the function call
	VanetRoutingExperiment experiment;
  //AnimationInterface anim ("scratch/Mobianimation.xml");
