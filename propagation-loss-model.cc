...
// ---------------------------Nawaz---------------------------------------------- //
//---------------------LOSnNLOS propagation loss model//

NS_OBJECT_ENSURE_REGISTERED (LOSnNLOSPropagationLossModel);

TypeId
LOSnNLOSPropagationLossModel::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::LOSnNLOSPropagationLossModel")
    .SetParent<PropagationLossModel> ()
    .AddConstructor<LOSnNLOSPropagationLossModel> ();
   // .AddAttribute ("Variable", "The random variable used to pick a loss everytime CalcRxPower is invoked.",
   //                StringValue ("ns3::ConstantRandomVariable[Constant=1.0]"),
   //                MakePointerAccessor (&LOSnNLOSPropagationLossModel::m_variable),
   //                MakePointerChecker<RandomVariableStream> ())
  //;
  return tid;
}
LOSnNLOSPropagationLossModel::LOSnNLOSPropagationLossModel ()
...
double
LOSnNLOSPropagationLossModel::DoCalcRxPower (double txPowerDbm,
                                           Ptr<MobilityModel> a,
                                           Ptr<MobilityModel> b) const
{
 ...
 // return txPowerDbm + rxc;
	...
	/*Hiue LOS/OLOS*/
	double mean = 1.0;
	double normDistance, LOSprob, OLOSProb;
	double plLOS;
	double plOLOS;
	double rxPowerDbm;
	...
	if(distance<200)
		{
		plLOS = 0.78067*10*log10(distance) + 70.36 ;
		plOLOS = 0.92174*10*log10(distance) + 79.7 ;
		}
	else
		{
		plLOS = 4.0251*10*log10(distance) -1.85;
		plOLOS = 3.1373*10*log10(distance) + 28.7;
		}

	//if obstructed line of sight use, rxPowerDbm = txPowerDbm-plOLOS
	//if obstructed line of sight use, rxPowerDbm = txPowerDbm-plLOS

	/*if(distance>150)
	rxPowerDbm = txPowerDbm-plOLOS;
	else */
	//rxPowerDbm = txPowerDbm-plLOS;

	//NS_LOG_UNCOND(" txPowerDbm="<<txPowerDbm<<" rxPowerDbm="<<rxPowerDbm);
	/************Probabilistic calculation for rxPowerDbm*************/
	/*normDistance = distance/100.0; //normalized the distance for exp distribution with 1 mean
	LOSprob = exp(-mean*normDistance);
	OLOSProb=1-LOSprob;
	rxPowerDbm = txPowerDbm-(plLOS*LOSprob+plOLOS*OLOSProb); */
	/************Probabilistic calculation for rxPowerDbm*************/
	/**End Hieu LOS/OLOS*/
	...
	unsigned seed = std::chrono::system_clock::now().time_since_epoch().count();
	  ...
	  std::normal_distribution<double> distribution (0.0,1.0); //gaussian distribution

	int urban_type = 1; //0:urban; 1:suburban
	double Tx_distance, Rx_distance, pl_NLOS;
	double DC= 100.0; //critical distance
	double n_NLOS=2.69;
	double sigma=4.1;
	double w_Tx, w_Rx,d_Tx;
	double lambda;

	txPowerDbm = 23+3;
	w_Tx=w_Rx=18;
	d_Tx=6;
	lambda= 3*pow(10,8)/(5.9*pow(10,9));
	double rand_number = distribution(generator);
	Vector pos_Tx, pos_Rx;
	pos_Tx = a->GetPosition ();
	Tx_distance= pos_Tx.x;
	pos_Rx = b->GetPosition ();
	Rx_distance= pos_Rx.x;
	//NS_LOG_UNCOND(" Tx_distance="<<Tx_distance<< " Rx_distance="<<Rx_distance);

	if((Tx_distance*Rx_distance)>0.0) //+tive means both vehicle on the same side, i.e. LOS
	{
		rxPowerDbm=0.0;
		//NS_LOG_UNCOND("rxPowerDbm="<<rxPowerDbm);
	}
	else //vehicles are on the opposite sides, i.e., NLOS
	{
	//Finding absolute position
	Tx_distance= std::abs(pos_Tx.x);
	Rx_distance= std::abs(pos_Rx.x);

	if (Rx_distance<DC)
		pl_NLOS = 3.75 + urban_type*2.94 + 10*log10(pow(((pow(Tx_distance,0.957)/pow((d_Tx*w_Rx),0.81))*(4*3.141*Rx_distance/lambda)),n_NLOS)) + rand_number;
	else
		pl_NLOS = 3.75 + urban_type*2.94 + 10*log10(pow(((pow(Tx_distance,0.957)/pow((d_Tx*w_Rx),0.81))*(4*3.141*pow(Rx_distance,2)/(lambda*DC))),n_NLOS)) + rand_number;

	rxPowerDbm=txPowerDbm-pl_NLOS;
	//NS_LOG_UNCOND("value="<<value);

	//NS_LOG_UNCOND("rxPowerDbm="<<rxPowerDbm<<" pl_NLOS="<<pl_NLOS<<" rand_number"<<rand_number);
	}//end else

	/*End Rahim's LOS/NLOS*/
	/**rcver's threshold manipulation*/
	if (rxPowerDbm>=-80.0)
		rxPowerDbm=0.0;
	else if (rxPowerDbm>=-85.0 && rxPowerDbm<-80.0)
		rxPowerDbm=-100.0;
	else
		rxPowerDbm=-1000.0;
	/**rcver's threshold manipulation*/
...
int64_t
LOSnNLOSPropagationLossModel::DoAssignStreams (int64_t stream)
{
...
// -------------------------------End Nawaz LOSnNLOSPropagationLossModel-------------------------------------- //
                                                 ...
                                                 Ptr<MobilityModel> b) const
{
  ...
  //std::cout<<"txAntHeight="<<txAntHeight<<" rxAntHeight"<<rxAntHeight;
