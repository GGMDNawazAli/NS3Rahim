...
/*Ming*/
class FwdHeader : public Header
{
public:
	FwdHeader (uint16_t fwd = 0);
  virtual ~FwdHeader ();
...
private:
  uint16_t m_fwd; ///< 1 or 0
};
static inline std::ostream & operator<< (std::ostream& os, const FwdHeader & packet)
{
  packet.Print (os);
  return os;
}
...
public:
  ...
  */
  ...
  /*Ming*/
  void SetForwarderNode(bool f);
  //bool GetForwarderNode(); //Ming
  bool GetForwarderNode(Ptr<Node> rxNode);//Nawaz

  void RelayReceivedBsmPacket (Ptr<Socket> socket, Ptr<Packet> p, uint32_t pktSize, uint32_t sendingNodeId);
  void ReceiveRelayedBsm(Ptr<Socket> socket);
 /*Ming*/
...
private:
  ...
  void HandleReceivedInvoBSMPacket (Ptr<Packet> p,Ptr<Node> txNode, Ptr<Node> rxNode );
